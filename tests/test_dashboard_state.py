...
The Reflex State's event handlers, driven directly as a signed-in
student against a temporary database.

This is the browser flow without the browser: open the form, type
values, submit, and check what the API now holds and what the
dashboard would show. Since v1.2 the State is a client of the HTTP
API: every handler here goes State -> ApiClient -> API -> storage,
with the test student's bearer token, and `ME` is that student's id
for seeding the database directly.
"""
...
def run(coro):
    """Drive an async handler to completion, as Reflex would."""
...
def fresh_state() -> DashboardState:
    """A State signed in as the test student, exactly as after a login."""
    state = DashboardState(_reflex_internal_init=True)
    state._auth_token = TOKEN
    state.authenticated = True
    state.user_email = "student@example.com"
    return state
...
def test_complete_removes_it_from_the_active_list_but_keeps_the_row():
    ...
    run(state.complete_assignment(row_id))
...
def test_generate_failure_keeps_the_dashboard_usable(monkeypatch):
    ...
    monkeypatch.setattr(plan_api, "generate_study_plan", boom)      # the engine fails inside the API
    state = fresh_state()
    slot_on(0, 16, 18)
    add_one(state)
    run(state.generate_study_plan())
...
def planned_state() -> DashboardState:
    ...
    slot_on(0, 16, 18)
    slot_on(1, 16, 18)
    run(state.load_data())                                 # pick up the seeded slots, as a page load would
    add_one(state, name="Math", days=3, hours="2", priority="MEDIUM")
...
def test_editing_an_assignment_invalidates_the_plan():
    state = planned_state()
    run(state.open_edit(state.assignments[0]["id"]))
...
def test_completing_an_assignment_invalidates_the_plan():
    state = planned_state()
    run(state.complete_assignment(state.assignments[0]["id"]))
    assert_plan_cleared(state, "assignments")
...
def test_regenerating_clears_the_stale_flag_and_uses_the_new_inputs():
    state = planned_state()
    run(state.complete_assignment(state.assignments[0]["id"]))       # Math (2h) done; Physics (1h) left
    run(state.generate_study_plan())
...
def test_completing_an_assignment_that_is_already_gone_does_not_invalidate_the_plan():
    ...
    run(state.complete_assignment(missing))
    assert state.has_plan is True and state.plan_stale is False
...
def test_save_failure_shows_a_message_and_keeps_the_form_open(monkeypatch):
    ...
    monkeypatch.setattr(assignments_api.storage, "add_assignment", boom)   # storage fails behind the API
    state = fresh_state()
    state.open_form()
    fill(state)
    run(state.submit_form())
...
def test_reloading_with_identical_data_keeps_the_plan_and_its_risks():
    state = planned_state()
    days_before, statuses_before = state.plan_days, state.plan_statuses
    run(state.load_data())
    assert state.has_plan is True and state.plan_stale is False and state.plan_message == ""
...
def test_no_data_behaviour_is_unchanged_by_the_safety_net():
    state = fresh_state()
    run(state.load_data())                                  # nothing stored, no plan: nothing to compare
    assert state.has_plan is False and state.plan_stale is False and state.plan_message == ""
    run(state.generate_study_plan())
