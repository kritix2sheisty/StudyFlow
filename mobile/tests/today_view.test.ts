...
import { todayLines } from "../src/history/view";
import { buildTodayView, minuteOf, nowMinute, TodayAnswer, todayTotals } from "../src/today/view";
...
test("no plan and a stale plan are their own kinds with nothing to list", () => {
  expect(buildTodayView(day([], "no_plan"), 600)).toMatchObject({ kind: "no_plan", date: "2026-09-13", rows: [], done: 0, total: 0, now: null, next: null });
  expect(buildTodayView(day([], "plan_stale"), 600).kind).toBe("plan_stale");
});

test("a fresh plan with nothing today is empty", () => {
  expect(buildTodayView(day([]), 600)).toMatchObject({ kind: "empty", date: "2026-09-13", rows: [], done: 0, total: 0, now: null, next: null });
});
...
test("each session is done, now, past or upcoming by the clock", () => {
  const view = buildTodayView(day([
    ...
    session("19:00", "20:00", false, "Bio"),    // the first still to come: next
  ]), 16 * 60 + 47);
  expect(view.kind).toBe("sessions");
  expect(view.rows.map((r) => [r.assignment, r.state])).toEqual([
    ["Math", "done"], ["Math", "past"], ["CS", "now"], ["Bio", "next"],
  ]);
...
test("the end minute is not inside the session", () => {
  ...
  expect(rows(16 * 60 - 1)).toBe("next");                              // the only session still to come
...
test("rows keep the session fields the Focus screen will need", () => {
  ...
  expect(view.rows[0]).toMatchObject({
    ...
    start: "16:00", end: "18:00", duration_minutes: 120, completed: false, state: "next",
  });
});
