 ...
 * assignment), then Today and Progress refresh so the tick, the count
 * and the hours agree everywhere. A refusal from the API (stale plan,
...
export async function completeSession(deps: CompleteDeps, block: Block): Promise<CompleteAnswer> {
  ...
  await Promise.allSettled([deps.today.load(), deps.progress.load()]);
  return answer;
}
